# Script para gerar builds das 4 versões do leo-interview
# Executa: .\build-versions.ps1

Write-Host "🚀 Iniciando build das versões..." -ForegroundColor Green

# Lista de commits dos backups
$commits = @("424b7a87", "c69804bc", "2d26dc53", "a16b474d")

# Garante que estamos no master antes de começar
git checkout master
...
foreach ($commit in $commits) {
    Write-Host "📦 Construindo versão para commit $commit..." -ForegroundColor Cyan
    ...
    Write-Host "📥 Instalando dependências para $commit..." -ForegroundColor Yellow
    pnpm install
    if ($LASTEXITCODE -ne 0) { 
        Write-Error "❌ Erro ao instalar dependências para $commit"
        break
    }
    ...
    Write-Host "✅ Build de $commit concluído!" -ForegroundColor Green
}
...
# Lista o conteúdo final da pasta dist
Write-Host "📁 Conteúdo final da pasta dist:" -ForegroundColor Cyan
Get-ChildItem -Path "dist" -Recurse | ForEach-Object { 
    Write-Host "  $($_.FullName.Replace((Get-Location).Path + '\', ''))" -ForegroundColor Gray
}

# Adiciona, commita e faz push
Write-Host "💾 Fazendo commit e push das versões..." -ForegroundColor Cyan
git add -f dist
git add -f .
git commit -m "feat: add all 4 version builds for multi-version Pages deployment"
git push origin master

Write-Host "🎉 Processo concluído! Aguarde o deploy do GitHub Pages." -ForegroundColor Green
